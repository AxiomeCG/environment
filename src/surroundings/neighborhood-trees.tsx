...
import { Box3, Color, Group, InstancedMesh, Matrix4, Mesh, MeshPhongMaterial, Quaternion, Vector3 } from 'three'
import type { BufferGeometry } from 'three'
import { NEIGHBORHOOD_TREE_BUDGET, type TreePlan, type TreeSpecies } from './neighborhood-decoration'
...
const PRESETS: Record<Species, string> = { oak: 'Oak Medium', ash: 'Ash Medium', pine: 'Pine Medium', aspen: 'Aspen Medium', bush: 'Bush 3' }
...
function getVariant(species: Species): Variant {
  ...
  for (const level of [0, 1, 2, 3] as const) {
    tree.options.branch.sections[level] = Math.min(tree.options.branch.sections[level], level === 0 ? 5 : level === 1 ? 3 : 2)
    tree.options.branch.segments[level] = Math.min(tree.options.branch.segments[level], level === 0 ? 5 : level === 1 ? 4 : 3)
  }
  ...
  tree.traverse((child) => {
    ...
    const material = new PresentationMaterial({ map: source.map, alphaMap: source.alphaMap,
      color: source.color, side: source.side, alphaTest: source.alphaTest,
      transparent: false, depthWrite: true, roughness: 1, metalness: 0 })
  ...
  for (const [species, placements] of buckets) {
    ...
    for (const part of variant.parts) {
      ...
      if (!mesh || mesh.instanceMatrix.count < placements.length) {
        ...
        mesh = createContextInstances(part.geometry, part.material, Math.max(NEIGHBORHOOD_TREE_BUDGET, placements.length))
        mesh.name = partName
        root.add(mesh)
      }
      ...
      triangles += (part.geometry.index?.count ?? part.geometry.attributes.position!.count) / 3 * placements.length
      bytes += mesh.instanceMatrix.array.byteLength + (mesh.instanceColor?.array.byteLength ?? 0) + (part.geometry.index?.array.byteLength ?? 0)
...
export function buildTreeInstances(
  ...
  const placements: Placement[] = plans.map((plan) => {
    ...
    return {
      ...plan,
      crownAspect,
      position: [plan.position[0], heightAt(plan.position[0], plan.position[1]), plan.position[1]] as const,
    }
  })
  for (const plan of horizonPlans) placements.push({ ...plan, whitening: 0.65 })
