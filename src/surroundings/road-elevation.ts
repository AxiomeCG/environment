...
export function createRoadGradedTerrain(
  ...
  for (const edge of Object.values(network.edges)) {
    ...
    const firstHeight = points[0]!.height, lastHeight = points.at(-1)!.height
    ...
    for (let index = 1; index < points.length; index += 1) {
      ...
      heights[index] = Math.max(heights[index - 1]! - rise, Math.min(heights[index - 1]! + rise, heights[index]!))
    }
    ...
    for (let index = 1; index < points.length; index += 1) {
      ...
      const bed: Roadbed = { x: a.x, z: a.z, dx, dz, lengthSquared: dx * dx + dz * dz,
        height: heights[index - 1]!, rise: heights[index]! - heights[index - 1]!, halfWidth }
      const reach = halfWidth + SHOULDER_BLEND
      for (let x = Math.floor((Math.min(a.x, b.x) - reach) / BUCKET_SIZE); x <= Math.floor((Math.max(a.x, b.x) + reach) / BUCKET_SIZE); x += 1) {
        let column = buckets.get(x)
        if (!column) { column = new Map(); buckets.set(x, column) }
        for (let z = Math.floor((Math.min(a.z, b.z) - reach) / BUCKET_SIZE); z <= Math.floor((Math.max(a.z, b.z) + reach) / BUCKET_SIZE); z += 1) {
  ...
  const heightAt = (x: number, z: number): number => {
    ...
    let inside = false, boundaryDistanceSquared = Infinity
    for (let i = 0, j = boundary.length - 1; i < boundary.length; j = i++) {
      ...
      boundaryDistanceSquared = Math.min(boundaryDistanceSquared, (x - a[0] - dx * t) ** 2 + (z - a[1] - dz * t) ** 2)
    }
    if (inside || boundaryDistanceSquared < 1e-12) return ground
    let weightedHeight = 0, weight = 0, influence = 0
    for (const bed of beds) {
      const t = Math.max(0, Math.min(1, ((x - bed.x) * bed.dx + (z - bed.z) * bed.dz) / bed.lengthSquared))
      const distanceSquared = (x - bed.x - bed.dx * t) ** 2 + (z - bed.z - bed.dz * t) ** 2
      if (distanceSquared >= (bed.halfWidth + SHOULDER_BLEND) ** 2) continue
      const blend = smooth((bed.halfWidth + SHOULDER_BLEND - Math.sqrt(distanceSquared)) / SHOULDER_BLEND)
    ...
    return weight > 0
      ? ground + (weightedHeight / weight - ground) * influence * smooth(Math.sqrt(boundaryDistanceSquared) / 3)
      : ground
  }
  return {
    heightAt,
