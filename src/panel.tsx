...
import { ActionButton, subscribeCameraPose, useEditor } from '@pascal-app/editor'
...
import { SURROUNDINGS_PRESET_IDS, SURROUNDINGS_PRESET_POLICIES } from './surroundings/presets'
...
export default function EnvironmentPanel() {
  ...
  const surfaceActive = activeTool === SURFACE_MATERIAL_TOOL && editorMode === 'build'
  const surfaceSelected = selectedIds.some(
    (id) => (nodes[id as AnyNodeId]?.type as string | undefined) === SURFACE_MATERIAL_TOOL,
  )
  ...
  const siteAvailable = Object.values(nodes).some((node) => node.type === 'site')
  ...
  const activatePond = () => {
    activatePondTool(useEditor.getState() as unknown as GroundCoverToolActionTarget)
  }
  const activateRiver = () => {
    activateRiverTool(useEditor.getState() as unknown as GroundCoverToolActionTarget)
  }
  const activateWater = () => {
    ...
    return (
      <PaintPanel
        ...
        resumeLabel={
          waterTab === 'pond'
            ? pondSelected
              ? 'Resume selected pond'
              : 'Resume pond tool'
            : riverSelected
              ? 'Resume selected river'
              : 'Resume river tool'
        }
...
function GroundCoverPaintControls() {
  ...
  const setHeightAmount = useEnvironmentStore((state) => state.setGroundCoverHeightAmount)
  ...
  const replaceField = (density: number, color = brush.color) => {
    ...
    if (!target) return
    const paintField = createGrassPaintField(siteBounds(target.site.polygon.points), color, density)
  ...
  const resetHeight = () => {
    const target = resolveGroundCover()
    if (!target) return
    const heightField = createGrassHeightField(siteBounds(target.site.polygon.points))
      ...
      <details className="group rounded-lg border border-sidebar-border">
        ...
        <div className="flex flex-col gap-3 px-3 pb-3">
          ...
          <ParameterRange
            ...
            onChange={(noiseAmount) => setBrush({ noiseAmount: noiseAmount / 100 })}
          />
          <p className="text-xs leading-relaxed text-sidebar-foreground/60">
            ...
            ? [
                {
                  ...
                  description: 'Reset all locally painted grass heights across the Site.',
...
function BrushSection({ title, children }: { title: string; children: ReactNode }) {
  return (
...
function WholeSiteActions({
  ...
  useEffect(() => {
    if (pending !== null)
      container.current?.querySelector<HTMLButtonElement>('[data-site-cancel]')?.focus()
  ...
  return (
    <div className="flex flex-col gap-2 border-t border-sidebar-border pt-4" ref={container}>
      ...
      <p className="text-[11px] leading-relaxed text-sidebar-foreground/60" role="status">
...
function SurroundingsControls() {
  ...
  const preset = useEnvironmentStore((state) => state.surroundingsPreset)
  const setPreset = useEnvironmentStore((state) => state.setSurroundingsPreset)
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {!enabled && (
        <p className="text-xs text-sidebar-foreground/60" role="status">
          Surroundings are hidden. Turn them on to preview your changes.
        </p>
      )}
      {preset === 'regional' ? (
        <FrontageSelector
        ...
        <section className="flex flex-col gap-2 rounded-lg border border-sidebar-border p-3">
          <h3 className="text-xs font-semibold">Road layout</h3>
          <p className="text-xs leading-relaxed text-sidebar-foreground/70">
            {SURROUNDINGS_PRESET_POLICIES[preset].label} has no roads or homes. Switch to Regional
            to edit property-edge roads. Your road settings are kept.
          </p>
          <ActionButton
            className="min-h-10 focus-visible:outline-2 focus-visible:outline-ring"
            label="Use Regional roads"
            onClick={() => setPreset('regional')}
            type="button"
          />
        </section>
      )}
      ...
      <details className="group rounded-lg border border-sidebar-border">
        ...
        <div className="flex flex-col gap-3 px-3 pb-3">
          <label className="flex flex-col gap-2 text-xs">
            ...
            <input
              className="h-10 min-w-0 rounded-md border border-sidebar-border bg-sidebar px-3 focus-visible:outline-2 focus-visible:outline-ring"
              key={seed}
              defaultValue={seed}
              maxLength={80}
              onBlur={(event) => setSeed(event.currentTarget.value.trim() || 'pascal-suburbs')}
              onKeyDown={(event) => {
          ...
          <div className="divide-y divide-sidebar-border">
            <label
              className={`flex min-h-10 items-center justify-between gap-3 text-xs ${enabled ? 'cursor-pointer' : 'text-sidebar-foreground/50'}`}
            >
            ...
            <label
              className={`flex min-h-10 items-center justify-between gap-3 text-xs ${enabled && birdsEnabled ? 'cursor-pointer' : 'text-sidebar-foreground/50'}`}
            >
...
function SurfacePaintControls() {
  ...
  const setTextureSize = (value: number) => {
    ...
    useScene
      .getState()
      .updateNode(surface.id as AnyNodeId, { textureSize: value } as Partial<AnyNode>)
  }
  ...
  const presentation = SURFACE_MATERIAL_PRESENTATION.find((item) => item.id === material)!
  return (
    <div className="flex flex-col gap-5">
      <BrushSection title="Material">
        <div aria-label="Surface material" className="grid grid-cols-2 gap-2" role="group">
          {SURFACE_MATERIAL_PRESENTATION.map((item) => (
            <button
              ...
              type="button"
            >
              <img alt="" className="h-16 w-full object-cover" src={item.preview} />
              <span className="flex min-h-9 items-center justify-between gap-1 px-2 py-1.5 font-medium">
                {item.label}
                {material === item.id && (
                  <Check aria-hidden size={14} className="shrink-0 text-primary" />
                )}
              </span>
            </button>
          ))}
...
function getOrCreateGroundCover(): AnyNode | null {
  const scene = useScene.getState()
  let field = Object.values(scene.nodes).find((node) => (node.type as string) === GROUND_COVER_TOOL)
