	...
	if (activeSection === "ground-cover") {
		return (
			<div className="flex flex-col gap-4 p-4 text-sidebar-foreground">
				...
				{(!groundCoverActive || !groundCoverSelected) && (
					<button
						aria-label="Select Ground Cover layer"
						className="rounded-md border border-sidebar-border px-3 py-2 text-left text-xs transition-colors hover:bg-sidebar-accent"
						onClick={activateGroundCover}
						type="button"
					>
						{groundCoverActive ? "Select layer" : "Resume painting"}
					</button>
				)}
		...
		<div className="flex h-full min-h-0 flex-col text-sidebar-foreground">
			...
			{layerCount > 0 && (
				<div className="px-4 pb-3">
					<button
						aria-label="Select Ground Cover layer"
						className="flex w-full items-center justify-between rounded-md border border-sidebar-border px-3 py-2 text-left text-xs transition-colors hover:bg-sidebar-accent"
						onClick={() => selectEnvironmentTool("ground-cover")}
						type="button"
					>
						<span className="font-medium">Ground Cover</span>
						<span className="text-sidebar-foreground/50">
							{groundCoverSelected ? "Selected" : "Select layer"}
						</span>
					</button>
				</div>
			)}
